    ...
    try:
        ...
        for i, statement in enumerate(insert_statements):
            try:
                ...
                # Dịch từng text field (ngoại trừ Name và Model)
                ...
                for j, text_content in enumerate(matches):
                    if text_content and len(text_content.strip()) > 3:
                        ...
                        if is_english_text(text_content):
                            # Chỉ bỏ qua Name và Model thực sự (không phải Description)
                            # Name thường ngắn và không có dấu cách nhiều
                            # Model thường là tên ngắn như "Superocean", "Classic AVI", "Chronomat"
                            if (len(text_content.split()) <= 3 and 
                                ('Superocean' in text_content or 'Classic AVI' in text_content or 
                                 'Chronomat' in text_content or 'Heritage' in text_content)):
                                print(f"      ⏭️ Bỏ qua Model: {text_content[:50]}...")
                                continue
                            
                            # Bỏ qua URL
                            if text_content.startswith('http'):
                                print(f"      ⏭️ Bỏ qua URL: {text_content[:50]}...")
                                continue
                            
                            print(f"      🔄 Dịch {j+1}: {text_content[:50]}...")
                ...
                print(f"      ✅ Đã hoàn thành {i+1}/{len(insert_statements)}")
        ...
        # Ghi file kết quả
        output_file = "Products_Vietnamese_Final.sql"
        ...
        with open('translation_cache_final.json', 'w', encoding='utf-8') as cache_file:
            json.dump(translation_cache, cache_file, ensure_ascii=False, indent=2)
        
        print(f"💾 Đã lưu cache dịch thuật: translation_cache_final.json")
        ...
        for i, statement in enumerate(translated_statements):
            ...
            if len(matches) >= 3:
                print(f"   Name: {matches[0] if len(matches) > 0 else 'N/A'}")
                print(f"   Slug: {matches[1] if len(matches) > 1 else 'N/A'}")
                print(f"   Description: {matches[2][:100] + '...' if len(matches) > 2 and len(matches[2]) > 100 else matches[2] if len(matches) > 2 else 'N/A'}")
...
def main():
    print("🎯 SCRIPT DỊCH THUẬT SQL HOÀN CHỈNH")
    print("=" * 60)
