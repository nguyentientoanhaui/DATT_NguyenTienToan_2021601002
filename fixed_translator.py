...
class FixedSQLTranslator:
    def __init__(self, sql_file_path, output_file_path=None):
        self.sql_file_path = sql_file_path
        self.output_file_path = output_file_path or "Products_Vietnamese_Fixed.sql"
    ...
    def read_sql_file(self):
        ...
        try:
            ...
            # Tìm tất cả INSERT statements với pattern chính xác
            # Pattern này tìm INSERT statements kéo dài nhiều dòng
            ...
            # Chỉ lấy 3 statements đầu tiên để test
    ...
    def translate_file(self):
        ...
        print("🚀 BẮT ĐẦU DỊCH FILE PRODUCTS.SQL (FIXED)")
        print("=" * 60)
        ...
        for i, statement in enumerate(insert_statements):
            try:
                print(f"\n  [{i+1}/{total_statements}] Đang dịch...")
        ...
        try:
            ...
            with open('translation_cache_fixed.json', 'w', encoding='utf-8') as cache_file:
                json.dump(self.translation_cache, cache_file, ensure_ascii=False, indent=2)
            
            print(f"💾 Đã lưu cache dịch thuật: translation_cache_fixed.json")
    ...
    def show_sample_results(self, num_samples=3):
        ...
        try:
            ...
            insert_pattern = r"INSERT\s+\[dbo\]\.\[Products\].*?VALUES\s*\(([^)]+)\)"
            matches = re.findall(insert_pattern, content, re.IGNORECASE | re.DOTALL)
...
def main():
    ...
    output_file_path = "Products_Vietnamese_Fixed.sql"
    
    translator = FixedSQLTranslator(sql_file_path, output_file_path)
    
    # Kiểm tra xem có cache không
    if os.path.exists('translation_cache_fixed.json'):
        try:
            with open('translation_cache_fixed.json', 'r', encoding='utf-8') as cache_file:
    ...
    if success:
        print("\n🎉 HOÀN THÀNH DỊCH THUẬT (FIXED)!")
        print("=" * 60)
        print("✅ Đã dịch 3 sản phẩm đầu tiên sang tiếng Việt")
