...
namespace Shopping_Demo.Services
{
    public class GroqAIService : IAIService
    {
        ...
        public GroqAIService(HttpClient httpClient, IConfiguration configuration, ILogger<GroqAIService> logger)
        {
            ...
            _modelName = _configuration["Groq:ModelName"] ?? "llama2-70b-4096";
...
Hãy trả lời câu hỏi của khách hàng một cách thông minh và hữu ích nhất.";
                ...
                {
                    messages.Add(new { role = "system", content = $"Ngữ cảnh cuộc trò chuyện:\n{context}" });
                }

                // Thêm câu hỏi của khách hàng
                ...
                _logger.LogInformation("Sending request to Groq API: {Model}", _modelName);
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    _logger.LogInformation("Groq API response: {Response}", responseContent);
        ...
        public async Task<bool> IsHealthyAsync()
        {
            try
            {
                ...
                var response = await _httpClient.GetAsync("https://api.groq.com/openai/v1/models");
    ...
    // Model classes for Groq API response
